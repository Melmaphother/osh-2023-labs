...
bool send_404(int fd) {
    static const std::string str = "HTTP/1.1 404 Not Found\r\nContent-Length: 13\r\n\r\n404 Not Found";
    return write(fd, (const void *)str.c_str(), str.length()) > 0;
}


bool send_500(int fd) {
    static const std::string str = "HTTP/1.1 500 Internal Server Error\r\nContent-Length: 25\r\n\r\n500 Internal Server Error";
    return write(fd, (const void *)str.c_str(), str.length()) > 0;
}
            ...
            if (events[i].data.fd == fd) {
                    ...
                    if (size > 0) {
                        ...
                        if (state == -1) {
                            ...
                            if (file_fd == -1) {
                                ...
                                //conn->response.size = lseek(file_fd, 0, SEEK_END);
                ...
                if (conn->response.status_code == 200) {
                    if (!conn->response.has_sent_header) {
                        std::ostringstream ss;
                        ss << "HTTP/1.1 200 OK\r\nContent-Length: " << conn->response.size
                           << "\r\n\r\n";
                        const std::string &header = ss.str();
                        ssize_t size = write(sock_fd, (const void *)header.c_str(), header.length());
                        if (size > 0) {
                    ...
                    while (true) {
                        ...
                        ssize_t size = sendfile(sock_fd, conn->response.fd, NULL, 8192);
                        if (size > 0) {
                ...
                if (is_finished) {
                    ...
                    */
                    ...
                    if (it == conn->request.get_headers().end()) {
                        delete conn;
                        close(sock_fd);
                        //std::cerr << "close" << sock_fd << " " << close(sock_fd) << "\n";
                    } else {
                        conn->request.clear();
                        conn->response.clear();
                        epoll_read(conn, epoll_fd);
